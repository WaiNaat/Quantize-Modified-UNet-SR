...
class Quant_ConvTranspose2d(Module):
    """
    class for quantizing nn.ConvTranspose2d in U-Net
    """

    def __init__(self, weight_bit, full_precision_flag=False):
        super(Quant_ConvTranspose2d, self).__init__()
        self.full_precision_flag = full_precision_flag
        self.weight_bit = weight_bit
        self.weight_function = AsymmetricQuantFunction.apply
    
    def __repr__(self):
        s = super(Quant_ConvTranspose2d, self).__repr__()
        s = "(" + s + " weight_bit={}, full_precision_flag={})".format(
            self.weight_bit, self.full_precision_flag)
        return s
